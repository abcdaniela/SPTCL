...
class UpBlock_dual(nn.Module):
    """Upssampling followed by ConvBlock"""

    def __init__(self, in_channels1, in_channels2, out_channels, dropout_p,
                 bilinear=True):
        super(UpBlock_dual, self).__init__()
        self.bilinear = bilinear
        if bilinear:
            self.conv1x1 = nn.Conv2d(in_channels1, in_channels2, kernel_size=1)
            self.up = nn.Upsample(
...
class EncoderWithContrastive(nn.Module):
    def __init__(self, params, contrastive_model):
        super(EncoderWithContrastive, self).__init__()
        self.encoder = Encoder(params)  # 使用之前的编码器结构
        self.contrastive_model = contrastive_model  # 对比学习模型

    def forward(self, x):
        feature = self.encoder(x)

        # 计算编码器的对比学习特征
        contrastive_embedding = self.contrastive_model(feature[-1])  # 使用编码器的最后一层特征

        # 将对比学习特征融合到特征图中
        feature[-1] = torch.cat((feature[-1], contrastive_embedding), dim=1)

        return feature


class UNet_CCT_Contrastive(nn.Module):
    def __init__(self, in_chns, class_num, contrastive_model):
        super(UNet_CCT_Contrastive, self).__init__()
        ...
        params = {'in_chns': in_chns,
                  ...
                  'bilinear': False,
                  'acti_func': 'relu'}
        self.encoder = EncoderWithContrastive(params, contrastive_model)
    ...
    def forward(self, x):
        ...
        aux1_feature = features[:3]  # 使用前三层特征进行辅助分割
...
class Decoder_attention(nn.Module):
    def __init__(self, params):
        ...
        self.up1 = UpBlock_attention(
            self.ft_chns[4], self.ft_chns[3], self.ft_chns[3], dropout_p=0.0)
        self.up2 = UpBlock_attention(
            self.ft_chns[3], self.ft_chns[2], self.ft_chns[2], dropout_p=0.0)
        self.up3 = UpBlock_attention(
            self.ft_chns[2], self.ft_chns[1], self.ft_chns[1], dropout_p=0.0)
        self.up4 = UpBlock_attention(
            self.ft_chns[1], self.ft_chns[0], self.ft_chns[0], dropout_p=0.0)
...
class Decoder_dual(nn.Module):
    def __init__(self, params):
        ...
        self.up1 = UpBlock_dual(
            self.ft_chns[4], self.ft_chns[3], self.ft_chns[3], dropout_p=0.0)
        self.up2 = UpBlock_dual(
            self.ft_chns[3], self.ft_chns[2], self.ft_chns[2], dropout_p=0.0)
        self.up3 = UpBlock_dual(
            self.ft_chns[2], self.ft_chns[1], self.ft_chns[1], dropout_p=0.0)
        self.up4 = UpBlock_dual(
            self.ft_chns[1], self.ft_chns[0], self.ft_chns[0], dropout_p=0.0)
    ...
    def forward(self, feature):
        ...
        x = self.up1(x4)  # Set the second argument to None to skip the connection
        x = self.up2(x)
        x = self.up3(x)
        x = self.up4(x)

        # Merge intermediate features with the final output
        output = self.out_conv(x)

        # You can use interpolation or other methods to match the spatial dimensions
        # of intermediate features with the final output and then concatenate them.
        # For example:
        x0 = F.interpolate(x0, size=output.size()[2:], mode='bilinear', align_corners=True)
        x1 = F.interpolate(x1, size=output.size()[2:], mode='bilinear', align_corners=True)
        x2 = F.interpolate(x2, size=output.size()[2:], mode='bilinear', align_corners=True)
        x3 = F.interpolate(x3, size=output.size()[2:], mode='bilinear', align_corners=True)
        merged_output = torch.cat((output, x0, x1, x2, x3), dim=1)
...
class UNet_CCT(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.main_decoder = Decoder_attention(params)
        self.aux_decoder1 = Decoder_attention(params)
...
#         self.encoder = Encoder(params)
#         self.main_decoder = Decoder(params)
#         self.aux_decoder1 = Decoder_dual(params)
#
#     def forward(self, x):
#         feature = self.encoder(x)
#         main_seg = self.main_decoder(feature)
