...
class BaseDataSets(Dataset):
    ...
    def get_labels(self):
        labels = []  # 存储提取的标签
        max_label_length = 0  # 用于存储最长的标签长度
        for idx in range(len(self.sample_list)):
            case = self.sample_list[idx]  # 获取案例名称

            if self.split == "train":
                h5f = h5py.File(self._base_dir +
                                "/ACDC_training_slices/{}".format(case), 'r')  # 打开训练集HDF5文件
            else:
                h5f = h5py.File(self._base_dir +
                                "/ACDC_training_volumes/{}".format(case), 'r')  # 打开测试集HDF5文件

            label = h5f['label'][:]  # 假设标签在HDF5文件的'label'数据集中

            if self.split != "train":
                # 将（10，256，216）的标签分割成十个（256，216）的标签
                num_slices = label.shape[0]
                for i in range(num_slices):
                    labels.append(label[i])

                # 更新最长的标签长度
                max_label_length = max(max_label_length, label.shape[1])

            else:
                # 更新最长的标签长度
                max_label_length = max(max_label_length, len(label))
                labels.append(label)  # 将提取的标签添加到列表中

        # 使用填充来统一标签长度
        padded_labels = [F.pad(torch.tensor(label), (0, max_label_length - label.shape[0]), value=0) for label in
                         labels]

        return padded_labels[1]
